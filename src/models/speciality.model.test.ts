...
describe('SpecialityModel Sequelize Model', () => {
  it('should have the correct table name', () => {
    expect(SpecialityModel.tableName).toBe('speciality_management');
  });

  it('should define all required fields with correct types and constraints', () => {
    const attributes = SpecialityModel.getAttributes();
    ...
    // Speciality name
    expect(attributes.speciality_name.type.toString({})).toContain('VARCHAR');
  ...
  it('should have freezeTableName and timestamps options set correctly', () => {
    expect(SpecialityModel.options.freezeTableName).toBe(true);
    expect(SpecialityModel.options.timestamps).toBe(false);
  });
});
