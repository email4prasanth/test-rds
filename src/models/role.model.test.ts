...
describe('RoleModel Sequelize Model', () => {
  it('should have the correct table name', () => {
    expect(RoleModel.tableName).toBe('user_role');
  });
  ...
  it('should define all required fields with correct types and constraints', () => {
    ...
    // Role name
    expect(attributes.role_name.type.toString({})).toContain('VARCHAR');
  ...
  it('should have freezeTableName and timestamps options set correctly', () => {
    expect(RoleModel.options.freezeTableName).toBe(true);
    expect(RoleModel.options.timestamps).toBe(false);
  });
});
